...
// ─────────────────────────────────────────────────────────────
//  Legacy product route using Sanity _id.
//  All content is dynamic from Sanity — zero hardcoded data.
//  Delegates tab rendering to <ProductTabs />.
// ─────────────────────────────────────────────────────────────
...
// ── UI-only defaults (no product data) ───────────────────────
...
export default function ProductPage() {
  ...
  // ── Fetch by slug first; fall back to _id lookup ──────────
  useEffect(() => {
    if (!productId) return
    ...
    async function fetchProduct() {
      setLoading(true)
      try {
        // Try slug route first (most common), then id fallback
        const res = await fetch(
          `/api/sanity/product?slug=${productId}`
        )
        ...
        if (data?.product) {
          ...
          const firstSize =
            data.product.sizes?.[0] ?? DEFAULT_SIZES[0]
          setSelectedSize(firstSize)
        }
      } catch (err) {
        console.error('[ProductPage/id] fetch error:', err)
      } finally {
        setLoading(false)
      }
    }
  ...
  const isWishlisted = displayProduct
  ...
  return (
    <MainLayout>
      ...
      {/* ── Dynamic Product Tabs — all content from Sanity ── */}
